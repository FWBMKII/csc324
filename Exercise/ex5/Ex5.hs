...
data Set a = Set [a] 
instance (Show a) => Show (Set a) where 
	show (Set a) = "{" ++ tail (take ((length (show a)) - 1) (show a)) ++ "}"
...
setIntersect (Set a) (Set b) = Set (filter (\x ->setIntersectHelper x b) 
	                                      a)


setIntersectHelper :: Eq a => a -> [a] -> Bool
setIntersectHelper _ [] = False
setIntersectHelper a b = if a == (head b)
                         then True
                         else setIntersectHelper a (tail b)